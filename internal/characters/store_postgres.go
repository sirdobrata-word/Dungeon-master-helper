...
// Схема: таблица characters(id text primary key, data jsonb not null)
...
func (s *PostgresStore) Create(sheet CharacterSheet) (CharacterSheet, error) {
	...
	const insertQuery = `INSERT INTO characters (id, data) VALUES ($1, $2::jsonb);`
	if _, err := s.db.Exec(insertQuery, sheet.ID, data); err != nil {
...
func (s *PostgresStore) Update(id string, sheet CharacterSheet) (CharacterSheet, error) {
	...
	const updateQuery = `UPDATE characters SET data = $2::jsonb WHERE id = $1;`
